# -*- coding: utf-8 -*-
"""
Link classes for URDF ROS1 exporter
...
class Link:
    def __init__(self, name, xyz, center_of_mass, repo, mass, inertia_tensor, rpy=None):
        ...
        xyz: [x, y, z] - coordinate for visual and collision
        center_of_mass: [x, y, z] - coordinate for center of mass
        repo: str - repository path for meshes
        mass: float - mass of the link
        inertia_tensor: [ixx, iyy, izz, ixy, iyz, ixz]
        rpy: [roll, pitch, yaw] - rotation for visual and collision (optional)
        """
        self.name = name
        self.xyz = [-_ for _ in xyz]  # reverse sign
        self.rpy = [-_ for _ in rpy] if rpy else [0, 0, 0]  # reverse sign
...
def make_inertial_dict(root, msg, base_link_name=None, progress=None):
    ...
    progress: ProgressReporter - optional progress window (one step per link)

    Returns
    ----------
    inertial_dict: {name: {mass, inertia, center_of_mass, xyz, rpy}}
    msg: str - Status message
    """
    inertial_dict = {}
    ...
    def process_occurrences(occurrences):
        for occs in occurrences:
            ...
            # World center of mass (meters) is needed for the parallel-axis
            # shift of the world-aligned inertia tensor...
            world_com_m = [_ / 100.0 for _ in prop.centerOfMass.asArray()]
            # ...while the URDF <inertial><origin> must be the COM expressed in
            # the link's LOCAL frame (meshes/links are in local coordinates).
            ...
            occs_dict['inertia'] = utils.origin2center_of_mass(moment_inertia_world, world_com_m, mass)
